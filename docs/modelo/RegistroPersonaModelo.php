...
private function verificarExistente(){
    $sql="select * from persona where nombres='$this->nombres'
                        and primerApellido='$this->primerApellido' 
                        and genero='$this->genero' and fechaNac='$this->fechaNac'";
...
$result=$this->conexion->ExecuteQuery($sql);
if($result){
    echo json_encode(array('Success'=>1,'Error'=>0,'Mensaje'=>'Registro correcto de la persona'));
}else{
    echo json_encode(array('Success'=>0,'Error'=>1,'Mensaje'=>'Hubo algun error al momento de registrar a la persona'));
}
}
